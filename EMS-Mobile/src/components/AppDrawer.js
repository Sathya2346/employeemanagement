...
const AppDrawer = ({ navigation, state, currentRoute }) => {
  ...
  const handleNavigate = (item) => navigation.navigate(isAdmin ? 'AdminMainTabs' : 'UserMainTabs', { screen: item.route });
  const isRouteActive = (itemRoute) => activeRouteName === itemRoute || activeRouteName === `${itemRoute}Tab` || itemRoute === `${activeRouteName}Tab`;
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scrollList} showsVerticalScrollIndicator={false}>
        {navItems.map((item) => {
          ...
          return (
            <TouchableOpacity key={item.route} style={[styles.navLink, isActive && styles.activeNavLink]} onPress={() => handleNavigate(item)} activeOpacity={0.7}>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: COLORS.primary, paddingVertical: 20, paddingHorizontal: 15 },
  header: { alignItems: 'center', paddingBottom: 16, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.25)', marginBottom: 8 },
  ...
  navLink: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, paddingHorizontal: 15, borderRadius: 8, marginVertical: 2 },
  activeNavLink: { backgroundColor: COLORS.primaryHover },
  icon: { marginRight: 12 },
  navLabel: { fontSize: 16, fontWeight: '500', color: COLORS.white },
  activeNavLabel: { fontWeight: '700', color: COLORS.white },
  logoutBtn: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, paddingHorizontal: 15, borderRadius: 8, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.25)', marginTop: 8 },
  logoutLabel: { fontSize: 15, fontWeight: '600', color: COLORS.white },
});
