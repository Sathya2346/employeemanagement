...
public class AdminController {
    ...
    public Map<String, Object> getAttendanceSummary() {
        ...
        // ✅ Get last 7 days (Sunday → Monday order)
        for (int i = 6; i >= 0; i--) {
            LocalDate date = today.minusDays(i);
            String dayName = date.getDayOfWeek().name().substring(0, 3).toUpperCase(); // e.g. SUN, MON
            days.add(dayName);

            List<Attendance> dailyRecords = attendanceService.getAttendanceByDate(date);
