...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ===== HELPERS =====
  const formatTimeDisplay = (date) => date ? date.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "--:--";
  const formatTimeForDB = (date) => date ? date.toTimeString().split(" ")[0] : null;
  const formatDateForDB = (date) => date.toISOString().split("T")[0];
  const formatDuration = (ms) => {
    if (!ms || ms < 0) return "00:00:00 Hrs";
...
async function loadUserAttendance() {
    ...
    data.forEach(record => {
        ...
        const status = record.sStatus ?? record.status ?? "Not Checked In"; // fallback
        updateTable(
            dateStr,
            record.checkInTime ?? "--:--",
            record.checkOutTime ?? "--:--",
            record.status ?? "Not Checked In" // ✅ BACKEND STATUS
        );
    });
}
window.addEventListener("load", loadUserAttendance);
          ...
          cell.addEventListener("click",async()=>{
            ...
            try{
              ...
              if(data && data.length>0){
                data.forEach(record=>{
                  const row=document.createElement("tr");
                  row.innerHTML=`<td>${new Date(record.attendanceDate).toLocaleDateString()}</td><td>${record.checkInTime||"--:--"}</td><td>${record.checkOutTime||"--:--"}</td><td>${record.status}</td>`;
  ...
  const updateTable=(date,checkIn,checkOut,status)=>{ const existing=Array.from(attendanceTable.rows).find(r=>r.cells[0].textContent===date); if(existing){ existing.cells[1].textContent=checkIn; existing.cells[2].textContent=checkOut; existing.cells[3].textContent=status; } else{ const row=document.createElement("tr"); row.innerHTML=`<td>${date}</td><td>${checkIn}</td><td>${checkOut}</td><td>${status}</td>`; attendanceTable.appendChild(row); } };
