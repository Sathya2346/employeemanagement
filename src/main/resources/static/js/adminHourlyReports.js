document.addEventListener("DOMContentLoaded", () => {
    ...
    // 🌟 CLIENT-SIDE PDF GENERATION (Global Level)
    const downloadBtn = document.getElementById("downloadPdfBtn");
    if (downloadBtn) {
        downloadBtn.addEventListener("click", () => {
            ...
            if (reportTableBody) {
                reportTableBody.querySelectorAll("tr").forEach(tr => {
                    ...
                    // Only add if it's a real data row, not the "No reports found" message
                    if (cells.length === 4) {
                        rows.push([
            ...
            doc.autoTable({
                ...
                body: rows,
                styles: { font: "helvetica", fontSize: 10, cellPadding: 3, valign: 'middle', halign: 'center' },
