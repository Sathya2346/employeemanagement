...
public class Comprehensive100PercentVerificationTest {
    ...
    public void testMeetingHeartbeatProofLoop() throws Exception {
        ...
        mockMvc.perform(post("/api/attendance/check-in/" + emp.getId())).andExpect(status().isOk());
        mockMvc.perform(post("/api/attendance/meetingin/" + emp.getId())).andExpect(status().isOk());
    ...
    public void testMultiUserConcurrentMeetingIsolation() throws Exception {
        ...
        mockMvc.perform(post("/api/attendance/check-in/" + empA.getId())).andExpect(status().isOk());
        ...
        mockMvc.perform(post("/api/attendance/check-in/" + empB.getId())).andExpect(status().isOk());
